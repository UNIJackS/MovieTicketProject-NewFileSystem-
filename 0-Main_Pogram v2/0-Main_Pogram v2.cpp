...
int main()
{
	// Calls the Program_Validator function which checks weather the snack and ticket info arrays are the same size and the credit card surcharge is positive 
	Program_Validator();
	
	for (int i = 0; i < max_tickets; i +=1) {
		// calls the string_checker function and asks for the users name and the returns it 
		string name = string_checker("What is your name ?", "Please enter your name. This can not be blank or a lone number");
		// calls the int_checker function and asks for the users age and returns it 
		int age = int_checker("what is your age ?", min_age, max_age);

		// Declears ticket_profit and ticket_price to be used as pointers for the ticket_cost_calculator function
		float ticket_profit, ticket_price;
		// Calls the ticket_cost_calculator which takes the age of the user and returns the ticket price and profit whitch are determined by the ticket ifo arrays
		ticket_cost_calculator(age, ticket_price, ticket_profit);
		

		// calls the snack_extractor which asks the user what snacks they would like and the quntity and saves it to the snack_info array
		snack_extractor(i);

		// calls the yes_or_no function and asks the user what payment method then returns it
		...
		// Declears snack_cost and snack_profit to be used in the csv_file_writter
		float snack_cost = 0, snack_profit = 0;
		...
		// calls the yes_or_no function and asks the user if they would like to sell another seat
		bool last_ticket = yes_or_no("would you like to sell another seat", "yes", "no", "Pleease enter yes or no");

		// Calls the csv_file_writter which writes all the users input into the csv file
		csv_file_writter(i, name, age, ticket_price, snack_cost, card, ticket_profit, last_ticket);

		// clears the command promt so that a new user can use the program
		system("CLS");
		cout << flush;

		// goes to the stop lable if it is the last ticket
		if (last_ticket == true) {
			break;
		}
	}
